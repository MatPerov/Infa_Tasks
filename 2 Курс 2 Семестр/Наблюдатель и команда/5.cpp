...
sf::Color getRandomColor()
{
    static std::random_device rd;
    static std::mt19937 gen(rd());
    std::uniform_int_distribution<int> d(0, 255); // Используем int вместо sf::Uint8
...
protected:
    ...
    std::vector<Command*> mCommands;
public:
    MultiCommand(ShapePool& pool, std::vector<Command*> Commands)
        : mPool(pool) {
        mCommands = Commands;
    }
    void execute() override
...
int main()
{
    ...
    control.addButton(new Button(window, { 40, 560, 240, 40 }, font, "All Random Colors"), new RandomAllColorsCommand(pool));
    ...
    control.addButton(new Button(window, { 40, 380, 240, 40 }, font, "Delete All Shapes"), new DeleteAllShapesCommand(pool));
    Command* p = new RandomAllColorsCommand(pool);
    Command* q = new RandomAllPositionsCommand(pool);
    control.addButton(new Button(window, { 40, 440, 240, 40 }, font, "MultiCommand"), new MultiCommand(pool, { p, q }));
